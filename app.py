...
import matplotlib.cm as cm # 1. ADDED: Import for Heatmap
...
# --- 2B. SESSION STATE INITIALIZATION (FIX FOR TOGGLE RESET) ---
...
# --- 6A. EXPLAINABILITY ENGINE (ADDED FOR HEATMAP) ---
...
# --- 7. UI LOGIC WITH STATE MANAGEMENT ---
...
# Demo Buttons now update SESSION STATE
with col_demo1:
...
if uploaded_files:
    ...
    current_files = [(f, f.name) for f in uploaded_files]
    if st.session_state.file_list != current_files:
         st.session_state.file_list = current_files

# Button now only updates state
if st.button("START ANALYSIS"):
    st.session_state.run_analysis = True

# Main Logic checks STATE, not just the button press
