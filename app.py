...
def create_pdf(img_path, status, confidence, filename):
    ...
    pdf.multi_cell(0, 5, "NOTE: This report is automatically generated by the PneuSight Deep Learning system by Artificial Intelligence Research Center (AIRC). It is intended solely as a preliminary triage tool to assist medical professionals. This document does not constitute a final medical diagnosis. All findings must be reviewed and verified by a licensed radiologist.")
...
with c2:
    st.markdown("""
        ...
        <p style='color:#666; font-size:1.1rem;'>Artificial Intelligence Research Center | Radiology Division</p>
    """, unsafe_allow_html=True)

st.divider()

uploaded_files = st.file_uploader("Upload Chest X-Rays (DICOM/JPEG)", type=["jpg", "jpeg", "png"], accept_multiple_files=True)
